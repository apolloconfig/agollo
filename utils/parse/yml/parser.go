...
// vp is a global Viper instance for YML parsing
// Using a single instance to improve performance
var vp = viper.New()

// init initializes the Viper instance with YML configuration type
func init() {
	vp.SetConfigType("yml")
}

// Parser implements the YML format parser for Apollo configuration system
// It provides functionality to parse YML format configuration content
// using the Viper library for robust YML parsing capabilities
type Parser struct {
}

// Parse converts YML format configuration content to a key-value map
// Parameters:
//   - configContent: The configuration content to parse, expected to be
//     a string containing YML formatted data
//
// Returns:
...
// 2. Uses Viper to parse YML content
...
//
// Returns:
//   - map[string]interface{}: Flattened key-value pairs from YML configuration
//
