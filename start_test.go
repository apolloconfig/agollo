...
func TestErrorStartMustReadFromRemote(t *testing.T) {
	t.SkipNow()
	server := runErrorResponse()
	newAppConfig := getTestAppConfig()
	newAppConfig.IP = server.URL
...
func TestErrorStartWithConfigMustReadFromRemote(t *testing.T) {
	...
	StartWithConfigMustReadFromRemote(func() (*config.AppConfig, error) {
		return newAppConfig, nil
	})

}

func TestStartMustReadFromRemote(t *testing.T) {
	t.SkipNow()
	c := appConfig
	handlerMap := make(map[string]func(http.ResponseWriter, *http.Request), 1)
	handlerMap["application"] = onlyNormalConfigResponse
	...
	client, _ := StartMustReadFromRemote()
...
func TestStartWithConfigMustReadFromRemote(t *testing.T) {
	...
	client, _ := StartWithConfigMustReadFromRemote(func() (*config.AppConfig, error) {
		return c, nil
	})
